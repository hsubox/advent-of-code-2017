...
function day7_part1(input) {
  ...
  input.split('\n').forEach(s => {
    if ((/(\w*) \((\d*)\) -> ([\w,\s]*)/g).test(s)) {
      const result = (/(\w*) \((\d*)\) -> (.*)/g).exec(s);
      const disc = result[1];
      const children = result[3].split(', ');
...
assert.equal(day7_part1(sampleInput), 'tknk');
console.log(day7_part1(input)); // uownj
...
function day7_part2(input) {
  ...
  input.split('\n').forEach(s => {
    if ((/(\w*) \((\d*)\) -> ([\w,\s]*)/g).test(s)) {
      const result = (/(\w*) \((\d*)\) -> (.*)/g).exec(s);
      const disc = result[1];
      const discWeight = Number(result[2]);
      const children = result[3].split(', ');
      tower[disc] = {discWeight, children};
    } else if ((/(\w*) \((\d*)\)/g).test(s)) {
      const result = (/(\w*) \((\d*)\)/g).exec(s);
      const disc = result[1];
      const discWeight = Number(result[2]);
      tower[disc] = {discWeight};
    }
  });

  function getChildrenTowerWeights(disc) {
    if (tower[disc].towerWeight) {
      // already calculated
    } else if (!tower[disc].children) {
      tower[disc].towerWeight = tower[disc].discWeight;
    } else {
      for (let i in tower[disc].children) {
        child = tower[disc].children[i];
        const {foundOddTower, foundOddTowerWeight} = getChildrenTowerWeights(child);
        if (foundOddTower) {
          return {foundOddTower, foundOddTowerWeight};
        }
      }

      const childrenTowerWeights = tower[disc].children.map((child) => tower[child].towerWeight);
      tower[disc].towerWeight = tower[disc].discWeight + childrenTowerWeights.reduce((a, b) => a + b, 0);

      if (childrenTowerWeights.some(childWeight => childWeight !== childrenTowerWeights[0])) {
        for (let i in tower[disc].children) {
          const child = tower[disc].children[i];
          if (tower[disc].children.every(otherChild => {
            return child === otherChild || tower[child].towerWeight !== tower[otherChild].towerWeight;
          })) {
            const otherTowerWeight = tower[disc].children[1] === child ? childrenTowerWeights[0] : childrenTowerWeights[1];
            const necessaryWeight = tower[child].discWeight + otherTowerWeight - tower[child].towerWeight;
            return {
              foundOddTower: child,
              foundOddTowerWeight: necessaryWeight,
            };
          }
        }
      }
  ...
  return getChildrenTowerWeights(day7_part1(input)).foundOddTowerWeight;
}

assert.equal(day7_part2(sampleInput), 60);
console.log(day7_part2(input)); // 596
