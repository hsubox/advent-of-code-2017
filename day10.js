...
function day10_part1(listSize, input) {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    for (let j = 0; j < input[i]; j++) {
      newList[(currentPosition + input[i] - 1 - j) % listSize] = list[(currentPosition + j) % listSize];
    }
    currentPosition = (currentPosition + input[i] + i) % listSize;
    list = newList;
  }
...
function day10_part1_modified(listSize, input, rounds) {
  ...
  for (let r = 0; r < rounds; r++) {
    for (let i = 0; i < input.length; i++) {
      ...
      currentPosition = (currentPosition + input[i] + skipSize) % listSize;
      skipSize += 1;
      list = newList;
    }
  }
...
function day10_part2(listSize, input) {
  ...
  for (let i = 0; i < 16; i++) {
    ...
    for (let j = 1; j < 16; j++) {
      denseHash[i] = denseHash[i] ^ sparseHash[16 * i + j];
    }
  }
