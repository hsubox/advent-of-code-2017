...
function day8_part1(input) {
  let registers = {};
  const inputPattern = /(\w*) (inc|dec) (-?\d*) if (\w*) (<|>|<=|>=|==|!=) (-?\d*)/;
  input.split('\n').forEach(s => {
    if (inputPattern.test(s)) {
      const result = inputPattern.exec(s);
      const r1 = result[1];
      const op1 = result[2];
      const v1 = Number(result[3]);
      const r2 = result[4];
      const op2 = result[5];
      const v2 = Number(result[6]);

      if (!registers.hasOwnProperty(r1)) {
        registers[r1] = 0;
      }

      if (!registers.hasOwnProperty(r2)) {
        registers[r2] = 0;
      }
  ...
  return Math.max(...Object.keys(registers).map((r) => registers[r]));
}

assert.equal(day8_part1(sampleInput), 1);
console.log(day8_part1(input)); // 6828
...
function day8_part2(input) {
  ...
  input.split('\n').forEach(s => {
    if (inputPattern.test(s)) {
      ...
      max = Math.max(max, ...Object.keys(registers).map((r) => registers[r]));
    }
  });
...
assert.equal(day8_part2(sampleInput), 10);
console.log(day8_part2(input)); // 7234
