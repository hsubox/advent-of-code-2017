...
function day4_part1(input) {
  return input.split('\n').filter((passphrase) => {
    return !passphrase.split(' ').sort().some((word, idx, words) => {
      return word === words[idx + 1];
    });
  }).length;
}

assert.equal(day4_part1('aa bb cc dd ee'), 1);
assert.equal(day4_part1('aa bb cc dd aa'), 0);
assert.equal(day4_part1('aa bb cc dd aaa'), 1);
console.log(day4_part1(input)); // 451

function day4_part2(input) {
  return input.split('\n').filter((passphrase) => {
    return !passphrase.split(' ').map((word) => {
      return word.split('').sort().join('');
    }).sort().some((word, idx, words) => {
      return word === words[idx + 1];
    });
  }).length;
}

assert.equal(day4_part2('abcde fghij'), 1);
assert.equal(day4_part2('abcde xyz ecdab'), 0);
assert.equal(day4_part2('a ab abc abd abf abj'), 1);
assert.equal(day4_part2('iiii oiii ooii oooi oooo'), 1);
assert.equal(day4_part2('oiii ioii iioi iiio'), 0);
console.log(day4_part2(input)); // 223
