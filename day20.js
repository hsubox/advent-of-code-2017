...
const input = `p=<-3770,-455,1749>, v=<-4,-77,53>, a=<11,7,-9>
...
function day20_part1(input) {
  const accelerations = input.split('\n').map(s => {
    const result = /a=<(-?\d+),(-?\d+),(-?\d+)>/.exec(s);
    const ax = Number(result[1]);
    const ay = Number(result[2]);
    const az = Number(result[3]);
    return Math.abs(ax) + Math.abs(ay) + Math.abs(az);
  });

  return accelerations.indexOf(Math.min(...accelerations));
}

assert.equal(day20_part1(`p=<3,0,0>, v=<2,0,0>, a=<-1,0,0>
p=<4,0,0>, v=<0,0,0>, a=<-2,0,0>`), 0);
console.log(day20_part1(input)); // 376
...
class Particle {
  ...
  position() {
    return [this.px, this.py, this.pz].join(',');
  }
}

function day20_part2(input) {
  let uncollided = input.split('\n').map((s, i) => {
    const result = /p=<(-?\d+),(-?\d+),(-?\d+)>, v=<(-?\d+),(-?\d+),(-?\d+)>, a=<(-?\d+),(-?\d+),(-?\d+)>/.exec(s);
    const px = Number(result[1]);
    const py = Number(result[2]);
    const pz = Number(result[3]);
    const vx = Number(result[4]);
    const vy = Number(result[5]);
    const vz = Number(result[6]);
    const ax = Number(result[7]);
    const ay = Number(result[8]);
    const az = Number(result[9]);
    return new Particle(px, py, pz, vx, vy, vz, ax, ay, az);
  });

  for (let i = 0; i < 50; i++) {
    let positions = {};
    uncollided.forEach(particle => {
      particle.step();
    ...
    Object.keys(positions).forEach(position => {
      if (positions[position].length === 1) {
        uncollided.push(...positions[position]);
      }
    });
  }

  return uncollided.length;
}

assert.equal(day20_part2(`p=<-6,0,0>, v=<3,0,0>, a=<0,0,0>
p=<-4,0,0>, v=<2,0,0>, a=<0,0,0>
p=<-2,0,0>, v=<1,0,0>, a=<0,0,0>
p=<3,0,0>, v=<-1,0,0>, a=<0,0,0>`), 1);
console.log(day20_part2(input)); // 574
