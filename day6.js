...
function day6(input) {
  ...
  while (!seenStates.has(currentState.join(','))) {
    seenStates.add(currentState.join(','));
    steps += 1;

    const [max, idx] = currentState.reduce(([max, idx], n, i) => {
      return (n > max) ? [n, i] : [max, idx];
    }, [0, 0]);
    currentState[idx] = 0;
    for (let i = 0; i < max; i++) {
      currentState[(idx + i + 1) % currentState.length] += 1;
    }
  }
  return [steps, currentState];
}

assert.deepEqual(day6([0, 2, 7, 0]), [5, [2, 4, 1, 2]]);
console.log(day6(input)); // Part 1: 3156
assert.deepEqual(day6([2, 4, 1, 2]), [4, [2, 4, 1, 2]]);
console.log(day6([ 0, 13, 12, 10, 9, 8, 7, 5, 3, 2, 1, 1, 1, 10, 6, 5 ])); // Part 2: 1610; input is the ending sequence of part 1
